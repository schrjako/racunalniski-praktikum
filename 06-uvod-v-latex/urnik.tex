...
\[ \text{velikostTabele} = \text{številoUrNaDan} \cdot \text{\v steviloDelovnihDni} \]
...
kakovost urnika:
\[ k_1 \cdot u_1 + k_2 \cdot u_2 + \dots + k_{n-1} \cdot u_n \]
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   \begin{enumerate}
      ...
      \( \text{sre\v canja}_i \), ki ga sestavlja \( \text{predmet}_i \), profesor, ki ta predmet predava,
      ...
      \item Ko preverimo vse proste termine vseh predavalnic, vstavimo \( \text{sre\v canje}_i\)
...
spremembo (npr. zamenjava \( \text{sre\v canja}_i \) in \( \text{sre\v canja}_j \)) sedaj shranimo v
